  ...
  select * from ['Credit card transactions - Indi$']
...
from ['Credit card transactions - Indi$']
group by city),
total_spent as (select sum(amount) as total_amount from ['Credit card transactions - Indi$'])
select top 5 cte.*,round(total_spend/total_amount*100,2) as percebt_contribution
from cte 
...
with cte as (
select *,sum(amount) over(partition by card_type order by date) as total_spend
from ['Credit card transactions - Indi$']
--order by card_type,total_spend desc
)
...
select t1.city from(
select t.*,rank()over(partition by card_type order by total_spend) as rnk from(
select city,date,card_type,amount, sum(amount) over(partition by city,card_type order by date) as total_spend
from ['Credit card transactions - Indi$']) t )t1
where card_type ='Gold' and rnk=1
...
select distinct exp_type from ['Credit card transactions - Indi$'];

with cte as (
select city,exp_type, sum(amount) as total_amount from ['Credit card transactions - Indi$']
